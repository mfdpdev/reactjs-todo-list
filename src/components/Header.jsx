  ...
  return (
    <>
      <div className="w-full bg-flat shadow-md rounded-xl p-2 flex flex-col justify-around">
        ...
        <div className="w-full bg-slate-100/70 rounded-xl h-12 flex justify-around items-center py-7 mt-2">
          {days.map( (e, i) => {
            if(i == current.getDay()){
              return <div key={i} className="font-bold bg-white p-2 rounded-md shadow-md text-slate-800">{e.substring(0,2)}</div>
            }
